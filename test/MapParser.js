...
describe('MapParser', function () {
  ...
  context('special characters', function () {
    context('|| in trueString', function () {
      context('once', function () {
        it('converts to |', function () {
          var parser = new MapParser('[<x:1?||>]-[<x:2?||X >]-[<x:3? X||>]-[<x:4? X||X >]')
          parser.roomId = -1
      ...
      context('multiple times', function () {
        it('converts all occurrences to |', function () {
          var parser = new MapParser('[<x:1?||X||X >]-[<x:2? X||X||>]-[<x:3?||X||X||>]-[<x:4?X||X||X>]')
          parser.roomId = -1
    ...
    context('>> in falseString', function () {
      context('once', function () {
        it('converts to >', function () {
          var parser = new MapParser('[<x:1?X|>>>]-[<x:2?X|>>N >]-[<x:3?X| N>>>]-[<x:4?X| N>>N >]')
          parser.roomId = null
      ...
      context('multiple times', function () {
        it('converts all occurrences to >', function () {
          var parser = new MapParser('[<x:1?X|>>N>>N >]-[<x:2?X| N>>N>>>]-[<x:3?X|>>N>>N>>>]-[<x:4?X|N>>N>>N>]')
          parser.roomId = null
